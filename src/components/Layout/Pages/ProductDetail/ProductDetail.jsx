      ...
      <div className="container mx-auto">
        <div className="product-detail__wrapper px-5 py-6 grid grid-cols-1 md:grid-cols-2 gap-3 md:gap-6 lg:gap-8 items-start">
          <div className="product-detail__thumb space-y-4">
            ...
            <div className="product-detail__sub-images grid grid-cols-2 gap-3">
          ...
          <div className="product-detail__content space-y-3 py-3 md:py-1 lg:py-1 lg:sticky lg:top-3">
            ...
            <div className="product-detail__meta-info flex items-center gap-3">
            ...
            <div className="product-detail__color-grid my-3">
              ...
              <div className="product__detail-color-group flex items-center justify-start gap-2 mt-1">
                {product?.colors?.map((color) => (
                  <button
                    key={color}
                    aria-label={`Selected ${color}`}
                    onClick={() => setActiveColor(color)}
                    className={`product-detail__color-option border border-[#ccc] rounded-full p-[0.1rem] ${activeColor === color ? "ring-2 ring-slate-900 dark:ring-white" : ""}`}
                  >
                    <div
            ...
            <div className="product-detail__sizes py-2">
              ...
              <div className="product-detail__size-group flex items-center justify-start gap-[0.4rem] mt-1 dark:text-white">
                {product?.sizes?.map((size) => (
                  <button
                    key={size}
                    aria-label={`Selected ${size}`}
                    onClick={() => setActiveSize(size)}
                    className={`product-detail__size-option border border-[#ccc] py-[0.3rem] px-2 text-center shadow-sm rounded-md uppercase font-medium tracking-wide text-[0.7rem] lg:py-1 lg:px-7 lg:text-[0.8rem] ${activeSize === size ? "border-2 border-black dark:border-white" : ""}`}
                  >
                    {size || "N/A"}
                  </button>
                ))}
        ...
        {/* Related Products */}
        <div className="product-detail__related mt-3 mb-10 px-5 lg:px-0 lg:my-14">
          ...
          <div className="product-detail__related-bottom mt-4">
            <div className="product-detail__related-grid grid grid-cols-1 gap-2 lg:grid-cols-4 lg:gap-4">
              {relatedProducts.map((item) => (
                <article className="product-card border border-[#ccc] group bg-white rounded-md p-1 sm:p-4 transition-all duration-500 overflow-hidden shadow-sm">
                  {/* Product Thumbnail */}
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-[0.9rem] sm:text-[1rem] text-slate-500">
                          <span className="font-semibold text-slate-700">
                            {(item?.reviews?.[0]?.rating || 1).toFixed(1)}
                          </span>{" "}
                          ({item?.reviews?.length || 1} reviews)
                        </span>
                      </div>
                    </div>
                  </div>
